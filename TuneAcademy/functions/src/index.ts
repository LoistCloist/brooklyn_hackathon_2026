...
function requiredEnv(name: string): string {
   ...
   if (!value) throw new HttpsError("failed-precondition", `Missing server environment variable ${name}.`);
   return value;
}
...
async function refreshGoogleAccessToken(uid: string, connection: GoogleCalendarConnection): Promise<string> {
   if (connection.accessToken && (connection.accessTokenExpiresAt ?? 0) > Date.now() + 60_000) {
      return connection.accessToken;
   }
   if (!connection.refreshToken) {
      throw new HttpsError("failed-precondition", "Connect Google Calendar before creating Meet links.");
   }
   ...
   const token = await readJsonResponse<{ access_token: string; expires_in?: number; scope?: string }>(
...
export const createGoogleCalendarAuthUrl = onCall({ region: REGION }, async (request) => {
   ...
   await getFirestore().collection("googleCalendarOAuthStates").doc(state).set({ uid, createdAt: FieldValue.serverTimestamp() });
...
export const googleCalendarOAuthCallback = onRequest({ region: REGION }, async (req, res) => {
   ...
   try {
      ...
      const token = await readJsonResponse<{ access_token: string; refresh_token?: string; expires_in?: number; scope?: string }>(
         await fetch("https://oauth2.googleapis.com/token", {
            method: "POST",
            headers: { "Content-Type": "application/x-www-form-urlencoded" },
            body,
      ...
      const profile = await readJsonResponse<{ email?: string }>(
         await fetch("https://www.googleapis.com/oauth2/v2/userinfo", { headers: { Authorization: `Bearer ${token.access_token}` } }),
      );
...
export const createGoogleMeetLinksForEngagement = onCall({ region: REGION }, async (request) => {
   const uid = request.auth?.uid;
   if (!uid) throw new HttpsError("unauthenticated", "Sign in first.");
   const engagementId = String((request.data as { engagementId?: unknown })?.engagementId ?? "").trim();
   ...
   const eng = engSnap.data() as { learnerId: string; instructorId: string; meetings?: { startAt: unknown; endAt: unknown }[] };
   if (eng.instructorId !== uid) {
      throw new HttpsError("permission-denied", "Only the instructor can create Meet links for this booking.");
   }
   ...
   for (let sessionIndex = 0; sessionIndex < meetings.length; sessionIndex++) {
      ...
      const createdEvent = await readJsonResponse<any>(
         await fetch("https://www.googleapis.com/calendar/v3/calendars/primary/events?conferenceDataVersion=1&sendUpdates=all", {
            method: "POST",
      ...
      if (!meetingUri) throw new HttpsError("internal", "Google created the event but did not return a Meet link.");
...
export const analyzeRecording = onObjectFinalized({ region: REGION }, async (event) => {
   ...
   const overall = Math.round(
      (result.pitch_centre + result.pitch_stability + result.rhythm + result.tone_quality + result.note_attack) / 5,
   );
