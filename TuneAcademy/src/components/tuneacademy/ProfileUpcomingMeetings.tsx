...
import {
  useProfileUpcomingMeetings,
  type ProfileUpcomingSlot,
} from "@/hooks/useProfileUpcomingMeetings";
...
import { ArrowRight, CalendarDays, Video, XCircle } from "lucide-react";
...
export function ProfileUpcomingMeetings() {
  ...
  const { slots } = useProfileUpcomingMeetings(user?.uid ?? null, role);
  ...
  async function confirmCancel() {
    ...
    try {
      await cancelMeetingSession({
        uid: user.uid,
        engagementId: cancelTarget.engagementId,
        sessionIndex: cancelTarget.sessionIndex,
      });
    ...
    <>
      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className={`text-xs font-black uppercase tracking-[0.18em] ${brandTheme.teal}`}>
              Upcoming meetings
            </p>
          ...
          <CalendarDays className="h-8 w-8 shrink-0 text-[#ffd666]/90" aria-hidden />
        </div>
                ...
                >
                  <div className="min-w-0">
                    <p className="text-sm font-black text-[#fffdf5]">
                      ...
                      {slot.endAt.toLocaleTimeString(undefined, {
                        hour: "numeric",
                        minute: "2-digit",
                      })}
                    </p>
        ...
        <AlertDialogContent className="border-[#fffdf5]/15 bg-[#0b1510] text-[#fffdf5]">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-[#e8f4df]/65">
              This removes the session for you and {cancelTarget ? otherPartyLabel(cancelTarget) : "the other person"}.
              You can still message them to reschedule.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
