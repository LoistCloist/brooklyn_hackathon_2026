...
function RequestCard({
  ...
  async function onAccept() {
    setBusy("accept");
    try {
      await acceptTutoringRequest(id, instructorId);
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
        <div className="flex shrink-0 items-center gap-3">
          {snippet !== "loading" && snippet?.avatarUrl ? (
            <img
              src={snippet.avatarUrl}
              alt=""
              className="h-16 w-16 rounded-2xl border border-[#fffdf5]/20 object-cover"
            />
          ) : (
        ...
        <div className="min-w-0 flex-1 space-y-3">
          ...
          {data.message.trim() ? (
            <div className="rounded-xl border border-[#ffd666]/25 bg-[#ffd666]/8 p-3">
              <p className="text-[10px] font-black uppercase tracking-widest text-[#ffd666]/90">
    ...
    return (
      <div className="mt-10 rounded-2xl border border-[#fffdf5]/12 bg-[#fffdf5]/5 p-12 text-center">
        ...
        <p className="mt-2 text-sm text-[#e8f4df]/55">
          When a student sends a schedule, it will land here for you to review.
        </p>
  ...
  return (
    <div className="mt-2 space-y-4">
      <p className="text-sm leading-relaxed text-[#e8f4df]/60">
        {rows.length} pending request{rows.length === 1 ? "" : "s"}. Accepting reserves those weekly
        blocks on your calendar so other learners cannot pick the same times.
      </p>
