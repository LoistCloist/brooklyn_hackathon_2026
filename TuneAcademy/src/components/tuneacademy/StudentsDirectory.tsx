...
function inviteBadge(status: LearnerDirectoryRow["inviteStatus"]): {
  label: string;
  className: string;
} | null {
  ...
  if (status === "pending")
    return {
      label: "Invite sent",
      className: "border-[#ffd666]/50 bg-[#ffd666]/15 text-[#ffd666]",
    };
  if (status === "accepted")
    return { label: "Connected", className: "border-[#2fc5b5]/45 bg-[#2fc5b5]/12 text-[#a6eee3]" };
...
export function StudentsDirectory() {
  ...
  const filtered = useMemo(() => {
    ...
    let list = rows.filter((r) => {
      ...
      const email = (r.doc.email || "").toLowerCase();
      return name.includes(needle) || email.includes(needle);
    });
    if (sort === "reels") {
      list = [...list].sort(
        (a, b) =>
          b.reelCount - a.reelCount || (a.doc.fullName || "").localeCompare(b.doc.fullName || ""),
      );
    }
    return list;
    ...
    <>
        ...
        <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-wrap gap-3">
            <div className="rounded-xl border border-[#fffdf5]/14 bg-[#0b1510]/55 px-5 py-4">
              <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#e8f4df]/50">
                Pool
              </p>
      ...
      <div className="mt-8 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <span className="mr-1 text-[10px] font-black uppercase tracking-[0.18em] text-[#e8f4df]/40">
            Sort
          </span>
      ...
      {loading ? (
        ...
        <div className="mt-12 rounded-xl border border-red-500/30 bg-red-500/10 p-6 text-sm text-red-200">
          {error}
        </div>
      ) : (
        <motion.ul
                ...
                >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                          ...
                          <p className="truncate text-xs font-medium text-[#e8f4df]/50">
                            {row.doc.email}
                          </p>
                        </div>
                      ...
                      {row.instruments.length > 0 ? (
                        <div className="mt-2 flex flex-wrap gap-1">
                          ...
                          {row.instruments.length > 4 ? (
                            <span className="text-[10px] font-bold text-[#e8f4df]/40">
                              +{row.instruments.length - 4}
                            </span>
                          ) : null}
                        </div>
                  ...
                  <div className="relative mt-5 flex gap-2">
                    <Link
                      ...
                      search={{
                        displayName: row.doc.fullName?.trim(),
                        avatarUrl: row.doc.avatarUrl?.trim(),
                      }}
                      className={cn(
      ...
      {!loading && !error && filtered.length === 0 ? (
        <div className="mt-12 rounded-2xl border border-[#fffdf5]/12 bg-[#fffdf5]/5 p-10 text-center">
          ...
          <p className="mt-2 text-sm text-[#e8f4df]/55">
            Loosen filters or clear search to widen the house.
          </p>
          <Pill
            variant="ghost"
            size="sm"
