...
import {
  subscribeEngagementsForInstructor,
...
export function MyStudentsPanel() {
  ...
  const activeSeriesCount = (learnerId: string) => {
    ...
    return engagements.filter(
      (e) => e.data.learnerId === learnerId && isEngagementActive(e.data.meetings, now),
    ).length;
  };
    ...
    return (
      <div className="mt-10 rounded-2xl border border-[#fffdf5]/12 bg-[#fffdf5]/5 p-12 text-center">
        ...
        <p className="mt-2 text-sm text-[#e8f4df]/55">
          When you accept a tutoring request, that learner appears here while the series is still
          active.
        </p>
            ...
            >
              <div className="flex gap-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {row.email ? (
                    <p className="truncate text-xs text-[#e8f4df]/50">{row.email}</p>
                  ) : null}
