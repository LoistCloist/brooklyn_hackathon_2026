...
export function MessagesScreen({ initialChatId }: MessagesScreenProps) {
   ...
   const { conversations, loading: loadingInvites, error: inviteError } = useMessagingInvitations(uid);
   ...
   const chatIds = useMemo(() => conversationsAugmented.map((c) => c.chatId), [conversationsAugmented]);
   ...
   const leakageIssue = findLeakageIssue(draft);
      ...
      <div className="mt-8 grid h-[calc(100dvh-17rem)] min-h-[420px] gap-4 overflow-hidden lg:grid-cols-[minmax(0,300px)_1fr]">
         ...
         >
            <div className="border-b border-[#fffdf5]/10 px-4 py-3">
               <p className="text-xs font-semibold uppercase tracking-wide text-[#e8f4df]/50">Conversations</p>
            </div>
            {myRole === "learner" ? (
               <div className="flex gap-1 border-b border-[#fffdf5]/10 px-2 py-2" role="tablist" aria-label="Filter conversations">
                  {(
                     [
            ...
            <div className="min-h-0 flex-1 overflow-y-auto">
               {loadingInvites ? (
                  ...
                  <div className="flex flex-col items-center px-4 py-12 text-center">
                     <MessageSquareText className="h-10 w-10 text-[#ffd666]/70" strokeWidth={1.5} aria-hidden />
                  ...
                  <div className="flex flex-col items-center px-4 py-10 text-center">
                     ...
                     <p className="mt-2 text-xs leading-relaxed text-[#e8f4df]/55">
                        Try another filter, or choose <span className="text-[#e8f4df]/75">All</span> to see every thread.
                     </p>
                  </div>
               ) : (
                           ...
                           <li key={c.chatId}>
                              ...
                              >
                                 ...
                                 <div className="min-w-0 flex-1">
                                    <p className="truncate text-sm font-semibold text-[#fffdf5]">{labelFor(c)}</p>
         ...
         >
            {!selectedChatId ? (
               <div className="flex flex-1 flex-col items-center justify-center px-6 py-16 text-center">
                  <MessageSquareText className="h-10 w-10 text-[#a6eee3]/70" strokeWidth={1.5} aria-hidden />
               ...
               <>
                  <header className="flex items-center gap-2 border-b border-[#fffdf5]/10 px-3 py-3">
                     ...
                     <div className="relative h-10 w-10 shrink-0 overflow-hidden rounded-full bg-[#fffdf5]/10">
                        {selected && profiles[selected.otherUserId]?.avatarUrl ? (
                           <img src={profiles[selected.otherUserId]?.avatarUrl} alt="" className="h-full w-full object-cover" />
                        ) : (
                           <span className="flex h-full w-full items-center justify-center text-sm font-bold text-[#ffd666]">
                     ...
                     <div className="min-w-0">
                        <p className="truncate font-semibold text-[#fffdf5]">{selected ? labelFor(selected) : ""}</p>
                  ...
                  <div className="min-h-0 flex-1 space-y-3 overflow-y-auto px-3 py-4">
                     {loadingThread ? (
                        ...
                        <p className="py-8 text-center text-sm text-[#e8f4df]/55">No messages in this thread yet.</p>
                     ) : (
                        messages.map((m) => {
                  ...
                  <footer className="border-t border-[#fffdf5]/10 p-3">
                     <div className="flex gap-2">
                        ...
                        <Button
                           ...
                           onClick={() => void onSend()}
                           disabled={sending || !draft.trim() || Boolean(leakageIssue)}
                     ...
                     <div className="mt-2 flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                        {leakageIssue ? (
                           <p className="rounded-md border border-[#ffd666]/30 bg-[#ffd666]/10 px-3 py-2 text-xs text-[#ffd666]">
                              {leakageIssue} {leakageGuardMessage}
                           </p>
                        ) : (
                           <p className="text-xs text-[#e8f4df]/45">
                              Contact details, outside links, and payment handles stay protected here.
                           </p>
                        )}
                        <p className="text-right text-[10px] text-[#e8f4df]/45">{draft.length} / 2000</p>
                     </div>
                  </footer>
               </>
            )}
