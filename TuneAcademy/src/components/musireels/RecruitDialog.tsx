...
import { collection, doc, getDoc, getDocs, limit, orderBy, query, serverTimestamp, where, writeBatch } from "firebase/firestore";
...
export function RecruitDialog({ open, onOpenChange, reel, instructorId, instructorName, onSent }: Props) {
   ...
   useEffect(() => {
      ...
      const run = async () => {
         ...
         try {
            ...
            const repQuery = query(collection(db, "reports"), where("userId", "==", learnerId), orderBy("createdAt", "desc"), limit(1));
         ...
         <DialogContent className="max-h-[90vh] max-w-lg overflow-y-auto">
            ...
            {loading ? (
               ...
               <div className="space-y-4 text-sm">
                  ...
                  <div>
                     <p className="mb-1 text-xs font-medium uppercase tracking-wide text-muted-foreground">Send an invitation message</p>
                     <Textarea
                        value={message}
            ...
            <DialogFooter className="gap-2 sm:gap-0">
               ...
               <Button type="button" onClick={() => void send()} disabled={posting || loading || Boolean(leakageIssue)}>
