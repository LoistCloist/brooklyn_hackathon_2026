...
type InstructorProfileDoc = InstructorFirestoreDoc & {
...
function InstructorProfile() {
  ...
  const [i, setI] = useState<InstructorProfileDoc | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    setLoading(true);
    void getInstructorDoc(id).then((doc) => {
      setI(doc && doc.bio?.trim() ? (doc as InstructorProfileDoc) : null);
      setLoading(false);
    });
  }, [id]);
  ...
  const groupSeatSummary = useMemo(() => {
    ...
    const groupSlots = dedupeWeeklySlots(i.groupWeeklyAvailability ?? []);
  ...
  return (
    <AppShell>
      <main className="mx-auto w-full max-w-3xl px-5 pt-6 sm:pt-8">
        <header className="mb-4 flex items-center justify-between">
          <button
            type="button"
            onClick={() => nav({ to: "/app/instructors" })}
            className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-hairline bg-[#0b1510]/55 text-[#fffdf5] backdrop-blur transition-colors hover:bg-[#fffdf5]/8"
        ...
        <Card className="overflow-hidden border-[#fffdf5]/16 bg-[#fffdf5]/8 p-0 shadow-[0_24px_80px_rgba(0,0,0,0.28)] backdrop-blur">
          <div className="border-b border-[#fffdf5]/10 bg-[#0b1510]/40 px-5 py-8 text-center sm:px-8 sm:py-10">
            <Avatar
              initials={initialsFromName(i.fullName)}
              src={i.avatarUrl}
              size={104}
              className="mx-auto text-2xl ring-2 ring-[#ffd666]/35 ring-offset-4 ring-offset-[#0b1510]"
            />
            <h1 className="mt-5 text-3xl font-black tracking-tight text-[#fffdf5]">{i.fullName}</h1>
            <div className="mt-2 flex items-center justify-center gap-1.5 text-sm text-[#e8f4df]/70">
              <Star className="h-4 w-4 fill-[#ffd666] text-[#ffd666]" />
              <span className="font-semibold tabular-nums text-[#fffdf5]">
                {i.rating.toFixed(1)}
              </span>
            </div>
            <div className="mt-4 flex flex-wrap justify-center gap-1.5">
              {i.specialties.map((s: string) => (
                <span
                  key={s}
                  className="rounded-full border border-[#a6eee3]/25 bg-[#a6eee3]/10 px-3 py-1 text-[10px] font-black uppercase tracking-widest text-[#a6eee3]"
                >
                  {formatSpecialtyLabel(s)}
                </span>
              ))}
            </div>
            <p className="mt-4 text-sm font-black text-[#ffd666]">
          ...
          <div className="space-y-6 px-5 py-5 sm:px-8 sm:py-7">
            ...
            <section>
              <h2 className="mb-2 text-sm font-black tracking-tight text-[#fffdf5]">About</h2>
              <div className="rounded-xl border border-[#fffdf5]/12 bg-[#0b1510]/35 p-4 text-sm leading-relaxed text-[#e8f4df]/72">
                {i.bio}
              </div>
            </section>

            <section>
              <h2 className="mb-2 text-sm font-black tracking-tight text-[#fffdf5]">Reviews</h2>
              <div className="rounded-xl border border-[#fffdf5]/12 bg-[#0b1510]/35 p-4 text-sm text-[#e8f4df]/62">
                No reviews yet.
              </div>
            </section>

            {user?.uid === id ? <ProfileReceipts viewerUid={id} viewerRole="instructor" /> : null}
          </div>
        </Card>
      </main>
                ...
                {(i.sessionType === "both" ||
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {(i.sessionType === "solo" || i.sessionType === "both") && (
                        <button
                          type="button"
                      ...
                      {(i.sessionType === "group" || i.sessionType === "both") && (
                        <button
                          type="button"
                    ...
                    {(i.sessionType === "group" || i.sessionType === "both") && (
                      <div className="mt-3 rounded-xl border border-[#2fc5b5]/25 bg-[#2fc5b5]/10 p-3">
                        <div className="flex items-center justify-between gap-3">
                          <div>
                            <p className="text-xs font-black uppercase tracking-widest text-[#a6eee3]">
                              Group seats
                            </p>
                            <p className="mt-1 text-xs text-muted-foreground">
                              {groupSeatSummary.openSlots} open weekly time
                              {groupSeatSummary.openSlots === 1 ? "" : "s"}
                            </p>
                          </div>
                          ...
                          {[0, 1, 2].map((seat) => (
                            <span
                              ...
                              className={`h-1.5 rounded-full ${
                                seat <
                                Math.min(GROUP_SEAT_CAPACITY, groupSeatSummary.totalSeatsLeft)
                                  ? "bg-[#2fc5b5]"
                                  : "bg-[#fffdf5]/15"
                              }`}
                            />
                  ...
                  <div className="max-h-48 overflow-y-auto rounded-xl border border-hairline bg-muted/20 p-3">
                    <div className="flex flex-wrap gap-2">
                      {offeredSlots.map((s) => {
                        ...
                        const seatsLeft =
                          chosenSessionType === "group"
                            ? groupSeatsLeftForSlot(engagements, s)
                            : null;
                        return (
                          <button
                            key={slotKey(s)}
