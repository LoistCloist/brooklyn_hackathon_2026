...
function StudentsTab() {
  ...
  const title =
    tab === "pending" ? "Pending requests" : tab === "mine" ? "My students" : "Explore students";

  return (
    <AppShell>
      <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="min-w-0">
          <p className={`text-xs font-black uppercase tracking-[0.22em] ${brandTheme.gold}`}>
            Instructor workspace
          </p>
          <h1 className="mt-2 text-2xl font-black tracking-tight text-[#fffdf5] md:text-3xl">
            {title}
          </h1>
        </div>
        <div className="flex max-w-full flex-wrap gap-1 rounded-2xl border border-[#fffdf5]/14 bg-[#0b1510]/55 p-1 shadow-inner">
          <Link
            to="/app/students"
            search={{ tab: "explore" }}
            className={cn(
              "inline-flex min-w-0 flex-1 items-center justify-center gap-1.5 rounded-xl px-3 py-2.5 text-[10px] font-black uppercase tracking-[0.1em] transition sm:gap-2 sm:px-4 sm:text-xs sm:tracking-[0.14em]",
              tab === "explore"
                ? "bg-[#ffd666] text-[#11140c] shadow-sm"
                : "text-[#e8f4df]/55 hover:text-[#fffdf5]",
            )}
          >
      ...
      {tab === "pending" ? (
        <PendingSchedulingRequestsPanel />
      ) : tab === "mine" ? (
        <MyStudentsPanel />
      ) : (
        <StudentsDirectory />
